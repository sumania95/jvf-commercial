...
class Releasing_Details_Form_Product_Table_AJAXView(LoginRequiredMixin,View):
    ...
    def get(self, request):
        ...
        if search or start or end:
            data['form_is_valid'] = True
            data['counter'] = self.queryset.exclude(id__in = Releasing_Detail.objects.values('product_id').filter(releasing_id = releasing_id)).filter(Q(description__icontains = search)|Q(part_number__icontains = search),branch=self.request.user.user_type.branch).count()
            product = self.queryset.exclude(id__in = Releasing_Detail.objects.values('product_id').filter(releasing_id = releasing_id)).filter(Q(description__icontains = search)|Q(part_number__icontains = search),branch=self.request.user.user_type.branch).order_by('description','part_number')[int(start):int(end)]
